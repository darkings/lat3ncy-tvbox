...
public class JarLoader {
    ...
    // jar 是否含自杀/环境检测（load 时由 ProtectedInitJar 扫描得出），危险的 jar 不实例化任何 spider
    private final ConcurrentHashMap<String, Boolean> dangerousJars = new ConcurrentHashMap<>();
    ...
    private boolean load(String key, File file) {
        ...
        try {
            ...
            dangerousJars.put(key, protectedInitJar.check(file.getAbsolutePath()));
            Log.i(TAG, "load success key=" + key + ", dangerous=" + dangerousJars.get(key) + ", file=" + file.getAbsolutePath());
            return true;
        } catch (Throwable e) {
    ...
    private void invokeInit(DexClassLoader loader, String jar) {
        try {
            ...
            if (protectedInitJar.check(jar)) {
                ...
                Log.i(TAG, "echo-load initNormalJar file=" + jar);
    ...
    public Spider getSpider(String key, String api, String ext, String jar) {
        ...
        try {
            ...
            // 危险 jar（含 killProcess 自杀/包名检测）不实例化 spider，避免搜索时触发 jar 代码自杀
            if (Boolean.TRUE.equals(dangerousJars.get(jaKey))) {
                // 纯工具类（配置中心/本地视频）例外：自杀代码仅在 Init.init（已被
                // ProtectedInitJar 拦截不执行），Config/LocalFile 类本身无任何自杀调用，放行
                String clzName = className(api);
                if (!"Config".equals(clzName) && !"LocalFile".equals(clzName)) {
                    Log.i(TAG, "getSpider skip dangerous jar key=" + jaKey);
