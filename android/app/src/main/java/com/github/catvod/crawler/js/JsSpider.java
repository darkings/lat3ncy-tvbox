...
public class JsSpider extends Spider {
    ...
    private static final long JS_CALL_TIMEOUT_SECONDS = 15;
...
//        return executor.submit((FunCall.call(jsObject, func, args))).get();
        ...
        try {
            // 限时等待 JS 调用，避免死循环/慢请求的 JS 源永久占用搜索线程
            return submit(() -> Async.run(jsObject, func, args).get()).get(JS_CALL_TIMEOUT_SECONDS, TimeUnit.SECONDS);
