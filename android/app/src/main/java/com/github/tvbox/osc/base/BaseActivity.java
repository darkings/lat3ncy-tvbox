...
public abstract class BaseActivity extends AppCompatActivity implements CustomAdapt {
    ...
    /** 默认整页加载用内容动画；搜索页覆盖为 SearchLoadingCallback。 */
    protected Class<? extends Callback> getLoadingCallbackClass() {
        return LoadingCallback.class;
    }

    protected void showLoading() {
        if (mLoadService != null) {
            mLoadService.showCallback(getLoadingCallbackClass());
        }
    }

    protected boolean isLoading() {
        if (mLoadService != null && mLoadService.getCurrentCallback() != null) {
            return mLoadService.getCurrentCallback().equals(getLoadingCallbackClass());
        }
        return false;
    }
