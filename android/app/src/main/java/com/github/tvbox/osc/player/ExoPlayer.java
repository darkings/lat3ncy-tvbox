...
public class ExoPlayer extends ExoMediaPlayer {
    ...
    public ExoPlayer(Context context) {
        super(context);
        // 直播大分片（约 10s/8Mbps）不能按点播默认 50s 去攒缓冲，否则必然落后滑动窗口。
        // 起播 1s、重缓冲 2.5s：出画更快；最大 16s 仍够扛一次慢切片，又不会掉出 live window。
        // 字节上限提到 24MB，避免 8Mbps 源被 12MB 过早停读。
        setLoadControl(new DefaultLoadControl.Builder()
                .setBufferDurationsMs(4_000, 16_000, 1_000, 2_500)
                .setTargetBufferBytes(24 * 1024 * 1024)
                .setPrioritizeTimeOverSizeThresholds(true)
                .setBackBuffer(0, false)
                .build());
        setRenderersFactory(buildRenderersFactory(context));
        LOG.i("echo-exo-live-load-control");
    ...
    private RenderersFactory buildRenderersFactory(Context context) {
        ...
        // L02: 默认恢复异步队列；仅对已知异常设备/解码器禁用
        if (shouldDisableAsyncQueueing()) {
            try {
    ...
    /**
     * 是否需要禁用 MediaCodec 异步缓冲队列。
     * 默认 false，仅对已知异常设备/解码器组合返回 true。
     */
    private boolean shouldDisableAsyncQueueing() {
        // TODO: 根据设备型号、SoC、Android 版本、解码器名称维护白名单
        return false;
    }
