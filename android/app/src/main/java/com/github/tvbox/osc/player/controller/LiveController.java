...
public class LiveController extends BaseController {
    ...
    protected void initView() {
        super.initView();
        // 直播场景慢网文案叠加换源语义（QA 清单第 12 节：加载超过 3s 显示线路尝试状态）
        setLoadingHintText(getContext().getString(R.string.loading_slow_network_live));
    }
    ...
    public void onLongPress(MotionEvent e) {
        if (listener != null) listener.longPress();
        super.onLongPress(e);
    }
    ...
    protected void onPlayStateChanged(int playState) {
        ...
        if (listener != null) listener.playStateChanged(playState);
    }

    @Override
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if (e1 == null || e2 == null) return false;
        if (listener == null) return super.onFling(e1, e2, velocityX, velocityY);
