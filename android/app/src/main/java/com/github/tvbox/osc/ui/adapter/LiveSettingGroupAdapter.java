...
public class LiveSettingGroupAdapter extends BaseQuickAdapter<LiveSettingGroup, BaseViewHolder> {
    ...
    public void setFocusedGroupIndex(int focusedGroupIndex) {
        int nextFocusedGroupIndex = findPositionByGroupIndex(focusedGroupIndex) != -1 ? focusedGroupIndex : -1;
        if (this.focusedGroupIndex == nextFocusedGroupIndex) return;
        int previousFocusedPosition = findPositionByGroupIndex(this.focusedGroupIndex);
        this.focusedGroupIndex = nextFocusedGroupIndex;
        if (previousFocusedPosition != -1)
