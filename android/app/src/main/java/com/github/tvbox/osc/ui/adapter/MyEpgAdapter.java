...
public class MyEpgAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if(i < data.size()){
            ...
            textview.setTextColor(ContextCompat.getColor(context, R.color.md3_on_surface));
            timeview.setTextColor(ContextCompat.getColor(context, R.color.md3_on_surface));
            Log.e("roinlong", "getView: "+  i);
               if(i == this.defaultSelection){
                    wqddg_AudioWaveView.setVisibility(View.VISIBLE);
                    int selectedColor = ContextCompat.getColor(context, R.color.md3_primary);
                    textview.setTextColor(selectedColor);
                    timeview.setTextColor(selectedColor);
