...
public class BackupDialog extends BaseDialog {
            ...
            if (backup.exists()) {
                ...
                if (AppDataManager.restore(db)) {
                    ...
                    if (data != null) {
                        ...
                        ToastUtil.info(getContext(), "恢复成功,即将自动重启应用!");
                        ...
                        ToastUtil.error(getContext(), "Hawk恢复失败!");
                    }
                } else {
                    ToastUtil.error(getContext(), "DB文件恢复失败!");
                }
            }
        } catch (Throwable e) {
            e.printStackTrace();
        ...
        try {
            ...
            if (AppDataManager.backup(db)) {
                ...
                if (!FileUtils.writeSimple(jsonObject.toString().getBytes("UTF-8"), new File(backup, "hawk"))) {
                    backup.delete();
                    ToastUtil.error(getContext(), "备份Hawk失败!");
                } else {
                    ToastUtil.info(getContext(), "备份成功!");
                }
            } else {
                ToastUtil.error(getContext(), "DB文件不存在!");
                backup.delete();
            }
        } catch (Throwable e) {
            e.printStackTrace();
            ToastUtil.error(getContext(), "备份失败!");
        }
    }
    ...
    void delete(String dir) {
        try {
            ...
            ToastUtil.info(getContext(), "删除成功!");
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
