...
public class PlayFragment extends BaseLazyFragment {
    ...
    private static final long RESOLVE_PLAY_URL_TIMEOUT_MS = 90 * 1000L;
    private static final long SWITCH_LINE_PLAY_TIMEOUT_MS = 20 * 1000L;
    private MyVideoView mVideoView;
    private TextView mPlayLoadTip;
    private ImageView mPlayLoadErr;
    private ProgressBar mPlayLoading;
    /** 播放页缓冲调度器：300ms 延迟显示 + 3s 慢网提示 + reduced-motion 静态降级 */
    private LoadingIndicator mPlayLoadingIndicator;
    ...
    private void initView() {
        ...
        // 播放页不设独立慢网文案位：mPlayLoadTip 本身承担状态说明，spinner 只做延迟与降级
        mPlayLoadingIndicator = new LoadingIndicator(mPlayLoading, null);
        mPlayLoadErr = findViewById(R.id.play_load_error);
        mController = new VodController(requireContext());
        // 详情页预览在 controller 创建前就可能设过 previewMode，创建后同步一次，
        // 否则屏显（SCREEN_DISPLAY）在详情页预览里仍会显示
        ...
        mController.setListener(new VodController.VodControlListener() {
            ...
            @Override
            public boolean errReplay() {
                // 错误态由控制器 tv_error_panel 统一呈现（错误文案+重试/换线/返回详情），
                // 隐藏旧版居中 tip，避免与错误面板重叠；
                // 返回是否已被自动重试/换线接管，控制器据此决定是否弹出错误面板
                boolean handled = errorWithRetry("视频播放出错", false);
                hideTipOnUiThread();
                return handled;
            }
    ...
    private void showCastDialog() {
        if (TextUtils.isEmpty(webPlayUrl)) {
            ToastUtil.error(mContext, "暂无可投屏播放地址");
            return;
        }
    ...
    private String getCastTitle() {
        ...
        if (series != null && !TextUtils.isEmpty(series.name)) {
    ...
    void selectMyInternalSubtitle() {
        ...
        trackInfo = ((IjkMediaPlayer)mediaPlayer).getTrackInfo();
        if (trackInfo == null) {
            ToastUtil.error(mContext, "没有内置字幕");
            return;
        }
    ...
    /**
     * 以带 300ms 延迟的方式展示缓冲 spinner；{@code msg} 作为慢网提示文案，
     * 超过 3s 未结束时才会浮现。快速完成的请求完全不闪 Loading。
     */
    void setTip(String msg, boolean loading, boolean err) {
        ...
        requireActivity().runOnUiThread(new Runnable() { //影魔
            @Override
            public void run() {
                // 文案立即可见（它本身就是用户可读的状态说明），spinner 走延迟调度
                mPlayLoadTip.setText(msg);
                mPlayLoadTip.setVisibility(View.VISIBLE);
                if (loading) {
                    mPlayLoadingIndicator.show();
                } else {
                    mPlayLoadingIndicator.hide();
                }
    ...
    boolean errorWithRetry(String err, boolean finish) {
        if (isPlaybackStarted()) {
            cancelPlayTimeout();
            hideTipOnUiThread();
            return true;  // 已开播的瞬时错误不打断播放，视为已处理
        }
        if (!autoRetry()) {
            if (!isAdded()) return true;  // fragment 已分离，无需展示错误面板
            requireActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (previewMode) {
                        // 预览失败：由宿主页面回退显示海报，预览区域不再展示错误提示
                        hideTipOnUiThread();
                        EventBus.getDefault().post(new RefreshEvent(RefreshEvent.TYPE_PREVIEW_ERROR));
                        return;
                    }
                    if (finish) {
                        setTip(err, false, true);
                        ToastUtil.info(mContext, err);
                    } else {
                        setTip(err, false, true);
                    }
                }
            });
            return false;  // 自动重试/换线均未接管，需要展示错误面板
        }
        return true;  // 自动重试或换线已接管
    }
                    ...
                    if (finalUrl != null) {
                        ...
                        try {
                            ...
                            if (playerType >= 10) {
                                ...
                                String playTitle = mVodInfo.name + (vs == null || TextUtils.isEmpty(vs.name) ? "" : " " + vs.name);
                        ...
                        // 回片头：把历史起播进度交给 controller，由其判定是否提示"长按左键返回片头"
                        // 注意：内部会按 previewMode 守卫，预览小窗（非全屏）不启用该逻辑
                        mController.prepareStartTipIfResumed(playTimeoutBasePosition);
                        ...
                        // 详情页预览强制 SurfaceView：TextureView 在容器尺寸切换后会复用旧 SurfaceTexture 导致黑屏。
                        applyPreviewRenderFactory();
    ...
    private String maybeWrapHls(String url, HashMap<String, String> headers) {
        if (url == null || url.isEmpty()) return url;
        if (url.startsWith("data:") || url.startsWith("http://127.0.0.1") || url.startsWith("http://localhost")) return url;
        if (url.contains(".mpd") || url.contains("type=mpd")) return url;
        if (!url.toLowerCase().contains(".m3u8")) return url;
        if (!Hawk.get(HawkConfig.HLS_CACHE, true)) return url;
        String wrapped = com.github.tvbox.osc.util.HlsCacheProxy.wrap(url, headers);
            ...
            public void onChanged(JSONObject info) {
                ...
                if (info != null) {
                    try {
                        ...
                        if (parse || jx) {
                            // ===== 网盘参数直达服务器无损直连 =====
                            // jx=1：url 是网盘 Base64 参数（如 BY7b22...），不是可播地址。
                            // 本地解析器（type:0 web 嗅探）无法处理网盘参数，且按 flag 过滤后
                            // 常无可用解析器（15ms 即失败）。改为绕过 initParse 的解析器选择，
                            // 直达 doServerParse：服务器按 title/ep/ep_index 搜采集站直出 m3u8。
                            // parseFlag/webUrl 先赋值，doServerParse 依赖这两个字段构造请求。
                            if (jx) {
                                parseFlag = flag;
                                webUrl = url;
                                mController.showParse(false);
                                doServerParse(null, flag, url);
                            } else {
                                boolean userJxList = playUrl.isEmpty() && ApiConfig.get().getVipParseFlags().contains(flag);
...
//                    获取播放信息错误后只需再重试一次
    ...
    public boolean selectQuality(int position) {
        ...
        try {
            ...
            if (parse || jx) {
                // 同 onChanged：jx=1 网盘参数直达服务器无损直连（见上方注释）
                if (jx) {
                    parseFlag = flag;
                    webUrl = url;
                    mController.showParse(false);
    ...
    public void onDestroyView() {
        ...
        cancelPlayTimeout();
        // 清空缓冲调度的 Handler 队列，避免泄漏
        if (mPlayLoadingIndicator != null) mPlayLoadingIndicator.release();
    ...
    private void playNext(boolean isProgress) {
        triedLineFlags.clear();
        boolean hasNext;
        List<VodInfo.VodSeries> nextList = VodPlayData.getSeriesList(mVodInfo);
        if (nextList.isEmpty()) {
            hasNext = false;
        } else {
            hasNext = mVodInfo.playIndex + 1 < nextList.size();
        }
        if (!hasNext) {
            ToastUtil.info(requireContext(), "已经是最后一集了!");
            return;
        }else {
            mVodInfo.playIndex++;
        }
    ...
    private void playPrevious() {
        triedLineFlags.clear();
        boolean hasPre = true;
        List<VodInfo.VodSeries> previousList = VodPlayData.getSeriesList(mVodInfo);
        ...
        if (!hasPre) {
            ToastUtil.info(requireContext(), "已经是第一集了!");
            return;
        }
        mVodInfo.playIndex--;
        play(false);
    ...
    boolean tryNextLine() {
        ...
        for (int i = startLineIndex; i < lineFlags.size(); i++) {
            ...
            List<VodInfo.VodSeries> seriesList = VodPlayData.getSeriesList(mVodInfo, flag);
        ...
        if (isAdded()) {
            requireActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    ToastUtil.info(mContext, "自动切换线路: " + flagToSwitch);
                }
            });
        }
        // 切换到新线路
    ...
    private VodInfo.VodSeries getCurrentSeries(String flag, int index) {
        return VodPlayData.getSeries(mVodInfo, flag, index);
    }
    ...
    public void setPlayTitle(boolean show)
    {
        if(show){
            ...
            if(mVodInfo!=null){
                VodInfo.VodSeries titleSeries = getCurrentSeries(mVodInfo.playFlag, mVodInfo.playIndex);
                playTitleInfo = mVodInfo.name + (titleSeries == null || TextUtils.isEmpty(titleSeries.name) ? "" : " " + titleSeries.name);
            }
    ...
    public void play(boolean reset) {
        ...
        exitingPreview = false;
        VodInfo.VodSeries vs = getCurrentSeries(mVodInfo.playFlag, mVodInfo.playIndex);
        if (vs == null || !VodPlayData.isValidPlayUrl(vs.url)) {
            ToastUtil.error(requireContext(), "当前集不可播放");
            return;
        }
        // 选集边界：第一集隐藏"上一集"、最后一集隐藏"下一集"
        java.util.List<VodInfo.VodSeries> seriesList = VodPlayData.getSeriesList(mVodInfo);
        mController.updateEpisodeButtons(
                seriesList != null && mVodInfo.playIndex - 1 >= 0,
                seriesList != null && mVodInfo.playIndex + 1 < seriesList.size());
        EventBus.getDefault().post(new RefreshEvent(RefreshEvent.TYPE_REFRESH, mVodInfo));
        setTip("正在获取播放信息", true, false);
        // 标题：片名 + 集名（集名已含片名时去冗余）+ 集数 X/Y
        String epName = vs.name == null ? "" : vs.name.trim();
        String playTitleInfo;
        if (!epName.isEmpty() && epName.contains(mVodInfo.name)) {
            playTitleInfo = epName;
        } else {
            playTitleInfo = mVodInfo.name + (epName.isEmpty() ? "" : " " + epName);
        }
        if (seriesList != null && seriesList.size() > 0) {
            playTitleInfo += "  " + (mVodInfo.playIndex + 1) + "/" + seriesList.size();
        }
        mController.setTitle(playTitleInfo);
    ...
    private void initParse(String flag, boolean useParse, String playUrl, final String url) {
        ...
        if (useParse) {
            // 按当前平台过滤：真实出流测试没平台/用户没启用的解析器一律不显示、不使用
            java.util.List<ParseBean> usable = ApiConfig.get().getParseBeanListForFlag(flag);
            mController.updateParseListForFlag(flag);
            ParseBean def = ApiConfig.get().getDefaultParse();
            if (def != null && usable.contains(def)) {
                parseBean = def;
            } else if (!usable.isEmpty()) {
                parseBean = usable.get(0);
            }
            if (parseBean == null) {
    ...
    /**
     * 详情预览宿主一律用 SurfaceView，避免用户设置里的 TextureView 在小窗↔全屏时黑屏。
     */
    private void applyPreviewRenderFactory() {
    ...
    /**
     * 小窗/全屏切换后，等容器完成布局再重建并重绑 Surface。
     * 用两次 post：第一次等 LayoutParams 生效，第二次再换 render，避免旧尺寸 Surface 被复用。
     */
    public void rebindPreviewSurface() {
        if (mVideoView == null) {
            return;
        }
        applyPreviewRenderFactory();
    ...
    void markPlaybackStarted() {
        ...
        cancelPlayTimeout();
        if (previewMode) {
            EventBus.getDefault().post(new RefreshEvent(RefreshEvent.TYPE_PREVIEW_RESUME));
        }
    }
    ...
    /**
     * 供 DetailActivity 判断预览是否处于有效播放状态。
     * 播完、出错、被释放或从未起播时返回 false——此时直接进全屏只会黑屏，
     * 调用方应先重新起播（jumpToPlay）再进全屏。
     * 注意：isPlaybackStarted() 在播放完成后 playbackStarted 标志不会复位，
     * 且 hasPlaybackProgress() 可能因 getCurrentPosition() 残留旧进度而误判，
     * 因此这里必须显式排除 STATE_PLAYBACK_COMPLETED / STATE_ERROR / STATE_IDLE，
     * 只有处于 PREPARED / BUFFERED / PLAYING / PAUSED 等活跃状态才返回 true。
     */
    public boolean isPreviewPlaybackActive() {
        // 播放器视图不存在，必然没有活跃播放
        if (mVideoView == null) return false;
        // 获取当前播放状态（STATE_PLAYBACK_COMPLETED=5 等，见 VideoView 常量定义）
        int state = mVideoView.getCurrentPlayState();
        // 已播完：画面已黑，必须重新起播
        if (state == VideoView.STATE_PLAYBACK_COMPLETED) return false;
        // 出错或空闲：同样没有可继续的画面
        if (state == VideoView.STATE_ERROR || state == VideoView.STATE_IDLE) return false;
        // 其余情况沿用原有判断（含 playbackStarted 标志与进度兜底）
    ...
    private void doParse(ParseBean pb) {
        ...
        if (pb.getType() == 4) {
            ...
            // 官方源：type-0 解析器在 App 内 WebView 无法出流（手机 UA / WASM），统一走服务器
            if (isVipParseScenario()) {
                doServerParse(pb.getName(), parseFlag, webUrl);
                return;
            }
            setTip("正在嗅探播放地址", true, false);
            mHandler.removeMessages(MSG_PARSE_TIMEOUT);
            mHandler.sendEmptyMessageDelayed(MSG_PARSE_TIMEOUT, 90 * 1000);
            if(pb.getExt()!=null){
                // 解析ext
                try {
            ...
            // 官方源：Ponyo解析 就是服务器多解析器入口，带上当前平台 + 用户启用的解析器列表
            if (isVipParseScenario()) {
                doServerParse(null, parseFlag, webUrl);
                return;
            }
...
//                        errorWithRetry("解析错误", false);
    ...
    /** 当前是否官方源解析场景（平台在 vipParseFlags 里，flag 做别名归一化比较）。 */
    private boolean isVipParseScenario() {
        if (parseFlag == null || parseFlag.isEmpty()) return false;
        String norm = ParseBean.normFlag(parseFlag);
        if (norm.isEmpty()) return false;
        for (String f : ApiConfig.get().getVipParseFlags()) {
            if (norm.equals(ParseBean.normFlag(f))) return true;
        }
        return false;
    }

    /**
     * 官方源统一走服务器解析（Ponyo解析 type-1）：
     * name=指定只跑某个解析器（null=全部启用且当前平台可用的）；parsers=用户启用的解析器名；
     * flag=平台，服务器按真实出流测试结果过滤无用解析器。
     */
    /**
     * 从片名提取语言标记（四平台多语言版本统一）。
     * 腾讯: "小猪佩奇 第12季[普通话版]" -> "普通话"
     * 爱奇艺: "小猪佩奇 第12季 英文版" -> "英文"
     * B站: "蜡笔小新 第二季（中文）" -> "普通话"（中文=普通话桶）
     * 优酷/芒果: 标题通常不带语言标记 -> null（服务端按无语言处理，行为不变）
     * 普通内容: 无关键词 -> null
     */
    private String extractLangTag(String name) {
    ...
    private void doServerParse(String nameParam, String flag, String vipUrl) {
        ...
        setTip("正在解析播放地址", true, false);
        StringBuilder sb = new StringBuilder(ponyo.getUrl()).append(mController.encodeUrl(vipUrl));
        if (nameParam != null && !nameParam.isEmpty()) {
            sb.append("&name=").append(mController.encodeUrl(nameParam));
        }
        // ===== 无损云直连：把片名 + 集名 + 集序号带给服务器 =====
        // 服务端优先按"片名+集数"搜采集站直出 m3u8（parse:0 直接播放），
        // miss 时自动回退旧 Playwright 路径。旧服务端不认识这些参数会直接忽略，双向兼容。
        // mVodInfo 在 initParse 之前必然已赋值（detail 回调才有播放动作），仍做判空防护
        if (mVodInfo != null && !TextUtils.isEmpty(mVodInfo.name)) {
            ...
            // 集序号（从 1 开始）：集名无数字时（如"高清"/"正片"）的兜底匹配依据
            // 电影单集场景 playIndex=0 -> epIndex=1，服务端对单集条目直接取第一条
            sb.append("&ep_index=").append(mVodInfo.playIndex + 1);
        }
        String parsers = ApiConfig.get().getEnabledParserNamesForFlag(flag);
        if (!parsers.isEmpty()) {
            sb.append("&parsers=").append(mController.encodeUrl(parsers));
        }
        if (flag != null && !flag.isEmpty()) {
            sb.append("&flag=").append(mController.encodeUrl(flag));
        }
        HttpHeaders reqHeaders = new HttpHeaders();
        try {
            JSONObject jsonObject = new JSONObject(ponyo.getExt());
            HashMap<String, String> headerMap = getHeaders(jsonObject);
            if (headerMap != null) {
                for (String key : headerMap.keySet()) {
                    reqHeaders.put(key, headerMap.get(key));
        ...
        final String reqUrl = sb.toString();
        // 用 OkHttp 自定义 45s 超时（OkGo 全局只有 10s，优酷出流 >10s 会超时）
        okhttp3.OkHttpClient okClient = com.github.catvod.net.OkHttp.client(45000);
        ...
        try {
            ...
            if (headerMap != null) {
                for (String key : headerMap.keySet()) {
                    if (key != null && headerMap.get(key) != null) hb.add(key, headerMap.get(key));
                }
            }
        } catch (Throwable e) {
            e.printStackTrace();
    ...
    private void parseMix(ParseBean pb,boolean isSuper)
    {
        ...
        parseThreadPool.execute(new Runnable() {
            ...
            public void run() {
                if(isSuper){
                    // 优先 json 解析器（更可靠），失败或无 json 再回退 web iframe
                    setTip("超级解析中", true, false);
                    if (!json_jxs.isEmpty()) {
                        parseThreadPool.execute(new Runnable() {
                            @Override
                            public void run() {
                                JSONObject res = SuperParse.doJsonJx(json_jxs, webUrl);
                                if (res != null && res.has("url") && !res.optString("url").isEmpty()) {
                                    rsJsonJX(res, true);
                                } else {
                                    runSuperWeb(jxs, parseFlag, webUrl);
                                }
                            }
                        });
                    } else {
...
//                        errorWithRetry("解析错误", false);
    ...
    /** 超级解析的 web 兜底：并行 iframe 加载所有 web 解析器。 */
    private void runSuperWeb(LinkedHashMap<String, HashMap<String, String>> jxs, String parseFlag, String webUrl) {
        ...
        if (mSysWebView == null && mXwalkWebView == null) {
            ...
            if (!useSystemWebView) {
                XWalkUtils.tryUseXWalk(mContext, new XWalkUtils.XWalkState() {
                    ...
                    @Override
                    public void fail() {
                        ToastUtil.info(mContext, "XWalkView不兼容，已替换为系统自带WebView");
                        initWebView(true);
                        loadUrl(url);
                    }
                    ...
                    @Override
                    public void ignore() {
                        ToastUtil.info(mContext, "XWalkView运行组件未下载，已替换为系统自带WebView");
                        initWebView(true);
                        loadUrl(url);
                    }
                });
