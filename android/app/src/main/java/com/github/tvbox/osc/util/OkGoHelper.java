...
public class OkGoHelper {
    ...
    static void initExoOkHttpClient() {
        ...
        // API 默认 10s 读超时对直播大分片不够：10s/8Mbps 约 10MB，
        // 慢源首包或中途停顿会被掐断，表现为卡碟、反复缓冲。
        // 连接仍用较短超时，尽快放弃死主机；读超时放到 30s，整次调用不限时。
...
//        LOG.i("echo-initDnsOverHttps ips:"+ips);
    ...
    private static synchronized void setOkHttpSsl(OkHttpClient.Builder builder) {
        try {
            // G08: 默认使用系统证书验证；仅调试模式或用户明确开启时才信任所有证书
            boolean trustAll = Hawk.get(HawkConfig.DEBUG_OPEN, false);
            if (!trustAll) {
                // 系统默认 TLS 验证，不设置自定义 TrustManager/HostnameVerifier
                return;
            }
