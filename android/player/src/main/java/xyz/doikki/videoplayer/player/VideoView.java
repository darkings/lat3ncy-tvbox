...
public class VideoView<P extends AbstractPlayer> extends FrameLayout
    ...
    /**
     * 初始化视频渲染View。
     * 小窗↔全屏重绑时先挂上新 View，等新 Surface 就绪后再拆旧 View，
     * 避免中间出现“播放器已解绑、新 Surface 还没创建”的黑屏窗口。
     */
    protected void addDisplay() {
        final IRenderView oldRender = mRenderView;
        ...
        LayoutParams params = new LayoutParams(
                ...
                ViewGroup.LayoutParams.MATCH_PARENT,
                Gravity.CENTER);
        final View newView = mRenderView.getView();
        mPlayerContainer.addView(newView, 0, params);
        if (oldRender == null) {
            return;
        }
        detachOldRender(oldRender, newView);
    }

    /**
     * 新 Surface 可用后再移除旧 render。旧 View 的 release 不得再 setDisplay(null)，
     * 否则会把刚绑上的新 Surface 清掉。
     */
    private void detachOldRender(final IRenderView oldRender, View newView) {
        final Runnable detach = new Runnable() {
            ...
            public void run() {
                ...
                // 必须先丢掉旧 View 的播放器引用，再 removeView。
                // 否则 surfaceDestroyed 会 setDisplay(null)，把刚绑上的新 Surface 清掉。
                oldRender.release();
                View oldView = oldRender.getView();
                if (oldView != null && oldView.getParent() == mPlayerContainer) {
                    mPlayerContainer.removeView(oldView);
                }
            }
        };
        if (newView instanceof SurfaceView) {
            SurfaceHolder holder = ((SurfaceView) newView).getHolder();
            ...
            // 极端情况下 Surface 回调丢失，300ms 后仍要拆掉旧 View，避免叠两层。
            newView.postDelayed(detach, 300);
            return;
        }
        if (newView instanceof TextureView) {
            TextureView textureView = (TextureView) newView;
            if (textureView.getSurfaceTexture() != null) {
                newView.post(detach);
                return;
            }
            newView.postDelayed(detach, 200);
            return;
        }
        newView.post(detach);
    }
        ...
        if (isInPlaybackState() && !mMediaPlayer.isPlaying()) {
            ...
            if (renderView instanceof SurfaceView) {
                ...
                if (holder.getSurface() != null && holder.getSurface().isValid()) {
                    ...
                    // 新 Surface 未就绪时只等待 created，不要再 addDisplay()。
                    // 否则会拆掉即将生效的 Surface，并把播放器绑回已销毁的 holder。
                    holder.addCallback(new SurfaceHolder.Callback() {
                        @Override
                        public void surfaceCreated(SurfaceHolder createdHolder) {
                            if (mMediaPlayer != null) {
                                mMediaPlayer.setDisplay(createdHolder);
                            }
                            resumePlay();
                            createdHolder.removeCallback(this);
                        }
     ...
     * L07: 暴露渲染视图，供异步截图使用。
     */
    public xyz.doikki.videoplayer.render.IRenderView getRenderView() {
        return mRenderView;
    }
