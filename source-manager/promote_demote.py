...
def get_latest_scores(con: sqlite3.Connection, fp: str,
    ...
    rows = con.execute(
        "SELECT total_score, timeslot, scored_at FROM score_snapshot "
        "WHERE fingerprint=? AND scored_at >= datetime('now', ?) "
        "ORDER BY scored_at DESC",
        (fp, f"-{days} days")).fetchall()
    return [{"score": r[0], "timeslot": r[1], "scored_at": r[2]} for r in rows]
...
def get_distinct_timeslots_passed(con: sqlite3.Connection, fp: str,
    ...
    """近 N 天通过的不同时段数。"""
    rows = con.execute(
        "SELECT DISTINCT timeslot FROM score_snapshot "
        "WHERE fingerprint=? AND total_score > 0 "
        "AND scored_at >= datetime('now', ?)",
        (fp, f"-{days} days")).fetchall()
    return len(rows)


def get_allow_min_score(con: sqlite3.Connection) -> float:
    """获取当前正式源的最低分。"""
    row = con.execute(
        "SELECT MIN(s.total_score) FROM score_snapshot s "
...
def evaluate_promotion(con: sqlite3.Connection, fp: str) -> dict:
    ...
    min_allow = get_allow_min_score(con)
    allow_count = get_allow_count(con)

    # 名额未满直接晋级（只要通过硬性条件）
    if allow_count >= ALLOW_QUOTA and latest_score <= min_allow:
        return {"action": "hold",
                "reason": f"分数 {latest_score} ≤ 正式源最低分 {min_allow}"}
